...
class VRPSolver:
    ...
    and time-window constraints using OR-Tools CP-SAT backed routing."""
    ...
    def solve(self, time_limit_seconds: int = 30):
        ...
        logger.warning("No solution found", status=routing.status())
        return None
    ...
    def _add_battery_dimension(self, routing, manager):
        ...
        for vehicle_id in range(self.problem.num_vehicles):
            ...
            initial_battery = int(
                self.problem.initial_battery_pct[vehicle_id] * 10
            )
