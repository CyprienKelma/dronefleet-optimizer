...
/**
 * Inbound adapter that listens to Pub/Sub messages for optimizer decisions and routes them to the domain.
 */
@Slf4j
@Component
public class DecisionListener {
    ...
    public void handleDecisionMessage(String payload) {
        try {
            ...
            List<Position> route = dto.route().stream()
                    .map(p -> new Position(p.lat(), p.lon()))
                    .collect(Collectors.toList());

            assignMissionUseCase.assignMission(dto.droneId(), dto.orderId(), route);

            log.info("Successfully processed decision for drone {} and order {}", dto.droneId(), dto.orderId());
            ...
            // Business rejection should not be retried as it's a "permanent" failure of this decision
        } catch (Exception e) {
            log.error("Transient error processing optimizer decision message: {}. Will be retried.", e.getMessage(), e);
